...
private:
    ...
    bool parseAdtsHeader(uint8_t* in, struct AdtsHeader* res);  // 解析ADTS头部
    int getFrameFromAACFile(int fd, uint8_t* buf, int size);    // 从AAC音频流中读取帧数据（NALU）

private:
    std::string mFile;     // 音频流文件名
    int mFd;               
